...
/***/ function(module, exports, __webpack_require__) {
	...
	var App = function App() {
		...
		// Настройки
		this._config = __webpack_require__(70);
		this.XML = __webpack_require__(71);
		this.CSV = __webpack_require__(72);
		...
		this.init = function () {
			...
			/* >>> */console.groupCollapsed('App:Start');
	
			// Инициализируем внешний вид
			self.view = __webpack_require__(73);
			...
			/* >>> */console.groupEnd('App:Start');
		};
	
		this.convertYML2CSV = function (yml_url, CB) {
			...
			var YML2JS = function YML2JS(url, callback) {
				var xhr = new XMLHttpRequest();
				xhr.open('GET', self._config.proxy_url + url, true);
				xhr.send();
				xhr.onreadystatechange = function () {
					...
					var data = false;
					try {
						data = self.XML.parse(xhr.responseText).yml_catalog.shop;
					} catch (e) {
						alert('Не разобать YML...');
						setTimeout(function () {
			...
			YML2JS(yml_url, function (data) {
				var categories = {};
				var cats = [];
				cats.push(data.categories.category);
				if (data.categories.category[0]) {
					cats = data.categories.category;
				}
				for (var k in cats) {
					categories[cats[k]['@id']] = cats[k]['#text'];
				}
	
				var products = [];
				for (var _k in data.offers.offer) {
	
					var offer = data.offers.offer[_k];
					if (offer.categoryId) {
						offer.category = categories[offer.categoryId];
					}
	
					for (var i in offer) {
						if ((0, _typeof3.default)(offer[i]) == 'object') {
							delete offer[i];
						}
					}
				...
				var str = self.CSV.convert(products);
	
				var filename = yml_url.replace('http://', '').replace('https://', '').split('/')[0];
				download(filename + '.csv', str);
	
				CB(true);
			});
		}, this.countersInit = function () {
			(function (d, w, c) {
				(w[c] = w[c] || []).push(function () {
					try {
						w.yaCounter42272619 = new Ya.Metrika({
							id: 42272619,
							clickmap: true,
							trackLinks: true,
...
/***/ function(module, exports) {
	...
	var _config = {
		proxy_url: 'https://jsonproxy.alexstep.com/?reqUrl='
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	function xml2json(xml, tab) {
		var X = {
			...
			toJson: function toJson(o, name, ind) {
				...
				} else if (o == null) json += (name && ":") + "null";else if ((typeof o === "undefined" ? "undefined" : (0, _typeof3.default)(o)) == "object") {
					var arr = [];
					for (var m in o) {
	...
	module.exports = {
		parse: function parse(xml_str) {
			return JSON.parse(xml2json($.parseXML(xml_str)).replace('undefined', ''));
		}
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(1)))
...
/***/ function(module, exports, __webpack_require__) {
	...
	*/
	function toCsvValue(theValue, sDelimiter) {
		var t = typeof theValue === "undefined" ? "undefined" : (0, _typeof3.default)(theValue),
		    output;
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = {
		...
		init: function init() {
			...
			var convert = function convert(yml_url) {
				if (!yml_url || yml_url.length < 5) {
					alert('Укажи корректную ссылку, с http://');
					return false;
				}
	
				$('.load-url').remove();
				$('#app').addClass('loading');
				$('form#convert').before('<div class="load-url">\u0417\u0430\u0433\u0440\u0443\u0437\u043A\u0430 ' + yml_url + '</div>');
	
				App.convertYML2CSV(yml_url, function () {
					$('.load-url').remove();
					$('#app').removeClass('loading');
				});
			};
	
			if (window.location.search.indexOf('yml_url=') != -1) {
				convert(window.location.search.split('yml_url=')[1].split('&')[0]);
			}
	
			$('form#convert').on('submit', function (e) {
				convert($('input[name="yml_url"]').val());
				e.preventDefault();
				return false;
			});
	
			$('input[name="yml_url"]').on('focus', function (e) {
